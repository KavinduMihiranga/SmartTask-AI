...
async function getTaskCard(page, taskTitle) {
  return page.locator(`h4:has-text("${taskTitle}")`).locator('../../..');
}
...
test.describe('📋 Tasks', () => {
  ...
  test('should create a task and see AI sub-tasks', async ({ page }) => {
    ...
    await expect(page.locator(`h4:has-text("${taskTitle}")`)).toBeVisible({ timeout: 30000 });

    await expect(page.locator('.subtask-item').first()).toBeVisible({ timeout: 10000 });

    const taskCard = await getTaskCard(page, taskTitle);
    await expect(taskCard.locator('.subtask-item')).toHaveCount(3, { timeout: 10000 });
  });
  ...
  test('should delete a task', async ({ page }) => {
    ...
    await expect(page.locator(`h4:has-text("${taskTitle}")`)).not.toBeVisible({ timeout: 5000 });
  });

  test('should show task count in heading', async ({ page }) => {
    await login(page);
    await expect(page.locator('h3:has-text("All Your Tasks")')).toBeVisible();
  });
...
test.describe('✅ Sub-tasks', () => {
  ...
  test('should toggle sub-task to completed', async ({ page }) => {
    ...
    await expect(page.locator(`h4:has-text("${taskTitle}")`)).toBeVisible({ timeout: 30000 });
    await expect(page.locator('.subtask-item').first()).toBeVisible({ timeout: 15000 });

    const taskCard = await getTaskCard(page, taskTitle);
    const firstSubTask = taskCard.locator('.subtask-item').first();
  ...
  test('should toggle sub-task back to incomplete', async ({ page }) => {
    await login(page);
    const taskTitle = `Untoggle Task ${Date.now()}`;
    await page.fill('input[name="title"]', taskTitle);
  ...
  test('should persist sub-task status after page reload', async ({ page }) => {
    ...
    const reloadedCard = await getTaskCard(page, taskTitle);
    await expect(reloadedCard).toBeVisible({ timeout: 10000 });
    await expect(reloadedCard.locator('.subtask-item').first().locator('input[type="checkbox"]')).toBeChecked({ timeout: 5000 });
  });
